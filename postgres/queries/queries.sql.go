...
DELETE FROM person WHERE id = $1 RETURNING id, name, age, location
`

func (q *Queries) DeletePerson(ctx context.Context, id int32) (Person, error) {
...
UPDATE person SET name = $1, age = $2, location = $3 WHERE id = $4 RETURNING id, name, age, location
`
...
INSERT INTO person (name, age, location) VALUES ($1, $2, $3) ON CONFLICT (id) DO UPDATE SET name = $1, age = $2, location = $3 RETURNING id, name, age, location
`
...
func (q *Queries) UpsertPerson(ctx context.Context, arg UpsertPersonParams) (Person, error) {
	row := q.db.QueryRow(ctx, upsertPerson, arg.Name, arg.Age, arg.Location)
	var i Person
	err := row.Scan(
		&i.ID,
		&i.Name,
